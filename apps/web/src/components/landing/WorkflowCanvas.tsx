...
function StatusBadge({ status }: { status: Status }) {
  ...
  if (status === "running") {
    return (
      <span className="flex items-center gap-1 rounded bg-primary/15 px-1.5 py-0.5 font-mono text-[9px] text-primary">
  ...
  return (
    <span className="rounded bg-raised px-1.5 py-0.5 font-mono text-[9px] text-muted">
      skipped
    </span>
  );
}
...
function DesktopGraph() {
  return (
    <div className="relative hidden aspect-[1100/480] md:block">
      ...
      {/* Terminal state */}
      <div
        className="absolute flex -translate-x-1/2 -translate-y-1/2 items-center gap-2 rounded-md border border-ok/40 bg-ok/10 px-3 py-1.5 text-xs font-medium text-ok"
    ...
    <ol className="relative ml-3 space-y-3 border-l border-line md:hidden">
      {nodes.map((node, i) => (
        <li key={node.id} className="relative pl-6">
          <span
            className="absolute -left-[5px] top-1/2 h-2.5 w-2.5 -translate-y-1/2 border-2 border-ink bg-line"
            aria-hidden
          />
          <NodeCard node={node} pulse={node.status === "running"} />
          {i === nodes.length - 1 && (
      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <Reveal>
          <SectionHeader
            ...
            title="Every run, from trigger to worker."
            lede="Watch events move through your graph in real time. Each node reports its own status, latency, and retry count — down to the individual execution."
          />
        </Reveal>

        <Reveal delay={120} className="mt-12">
          <div className="relative overflow-hidden rounded-[10px] border border-line bg-surface/50 p-4 sm:p-6">
            <DesktopGraph />
            <MobileGraph />
